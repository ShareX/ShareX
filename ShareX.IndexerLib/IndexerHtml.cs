...
public class IndexerHtml : Indexer
{
    ...
    private string GetFolderNameRow(FolderInfo dir, int level)
    {
        ...
        string pathTitle = settings.DisplayPath ? settings.DisplayPathLimited ? dir.FolderPath.Substring(prePathTrim) : dir.FolderPath : dir.FolderName;
    ...
    {
        string css = settings.UseCustomCSSFile && !string.IsNullOrEmpty(settings.CustomCSSFilePath) && File.Exists(settings.CustomCSSFilePath)
            ? File.ReadAllText(settings.CustomCSSFilePath, Encoding.UTF8)
            : Resources.IndexerDefault;
