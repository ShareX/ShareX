...
public partial class ImageWatermarkWindow : Window
{
    private readonly ImageWatermarkViewModel _viewModel;

    public ImageWatermarkWindow()
    {
        _viewModel = new ImageWatermarkViewModel();
        DataContext = _viewModel;
        InitializeComponent();
        RequestedThemeVariant = ThemeManager.GetCurrentTheme();
        _viewModel.SelectFilesRequested = SelectFilesAsync;
        _viewModel.SelectWatermarkImageRequested = SelectWatermarkImageAsync;
    ...
    private async Task<string?> SelectWatermarkImageAsync()
    {
        IReadOnlyList<IStorageFile> files = await StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions
        {
            Title = Localization.Strings.IconConverterViewModel_Select_image_dialog,
            AllowMultiple = false,
            FileTypeFilter = [FilePickerFileTypes.ImageAll]
        });
