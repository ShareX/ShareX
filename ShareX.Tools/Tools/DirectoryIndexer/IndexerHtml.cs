...
namespace ShareX.Tools
{
    public class IndexerHtml : Indexer
    {
        ...
        public override string Index(string folderPath)
        {
            ...
            sbHtmlIndex.AppendLine(HtmlHelper.Tag("title", string.Format(Localization.Strings.IndexerHtml_Index_for, Path.GetFileName(folderPath))));
        ...
        private string GetFolderNameRow(FolderInfo dir, int level)
        {
            ...
            if (!dir.IsEmpty)
            {
                ...
                {
                    folderNameRow += string.Format(dir.TotalFileCount == 1 ? Localization.Strings.IndexerHtml_One_file : Localization.Strings.IndexerHtml_File_count, dir.TotalFileCount);
                }
                ...
                if (dir.TotalFolderCount > 0)
                {
                    ...
                    folderNameRow += string.Format(dir.TotalFolderCount == 1 ? Localization.Strings.IndexerHtml_One_folder : Localization.Strings.IndexerHtml_Folder_count, dir.TotalFolderCount);
                }
        ...
        {
            return string.Format(Localization.Strings.IndexerHtml_Generated_by_on, $"<a href=\"{Links.Website}\">ShareX Directory Indexer</a>", DateTime.UtcNow);
        }
