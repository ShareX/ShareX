...
internal class ScrollingCaptureManager : IDisposable
{
    ...
    private bool IsScrollReachedBottom(IntPtr handle)
    {
        ...
        return NativeMethods.GetScrollInfo(handle, (int)SBOrientation.SB_VERT, ref scrollInfo)
            ? scrollInfo.nMax == scrollInfo.nTrackPos + scrollInfo.nPage - 1
            : CompareLastTwoImages();
    }

    private bool CompareLastTwoImages()
    {
        return lastScreenshot != null && previousScreenshot != null ? ImageHelpers.CompareImages(lastScreenshot, previousScreenshot) : false;
    }
    ...
    private Bitmap CombineImages(Bitmap result, Bitmap currentImage)
    {
        ...
        Rectangle rect = new(ignoreSideOffset, result.Height - currentImage.Height, currentImage.Width - ignoreSideOffset * 2, currentImage.Height);
        ...
        if (matchCount > 0)
        {
            ...
            if (matchHeight > 0)
            {
                ...
                Bitmap newResult = new(result.Width, result.Height - ignoreBottomOffset + matchHeight);
