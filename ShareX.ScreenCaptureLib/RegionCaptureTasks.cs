...
public static class RegionCaptureTasks
{
    public static Bitmap GetRegionImage(RegionCaptureOptions options = null)
    {
        ...
        using RegionCaptureForm form = new(RegionCaptureMode.Default, newOptions);
        form.ShowDialog();
    ...
    {
        using (RegionCaptureTransparentForm regionCaptureTransparentForm = new())
        {
    ...
    public static PointInfo GetPointInfo(RegionCaptureOptions options, Bitmap canvas = null)
    {
        ...
        using RegionCaptureForm form = new(RegionCaptureMode.ScreenColorPicker, newOptions, canvas);
        form.ShowDialog();

        if (form.Result == RegionResult.Region)
        {
            PointInfo pointInfo = new();
    ...
    public static SimpleWindowInfo GetWindowInfo(RegionCaptureOptions options)
    {
        ...
        using RegionCaptureForm form = new(RegionCaptureMode.OneClick, newOptions);
        form.ShowDialog();

        return form.Result == RegionResult.Region ? form.SelectedWindow : null;
    }

    public static void ShowScreenColorPickerDialog(RegionCaptureOptions options)
    {
        Color color = Color.Red;
        ColorPickerForm colorPickerForm = new(color, true, true, options.ColorPickerOptions);
    ...
    public static void ShowScreenRuler(RegionCaptureOptions options)
    {
        ...
        using RegionCaptureForm form = new(RegionCaptureMode.Ruler, newOptions);
        form.ShowDialog();
    }
