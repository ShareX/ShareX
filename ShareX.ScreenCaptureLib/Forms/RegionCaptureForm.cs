...
using ShareX.ScreenCaptureLib.Shapes.Effect;
using ShareX.ScreenCaptureLib.Shapes.Tool;
...
namespace ShareX.ScreenCaptureLib.Forms;
...
public sealed class RegionCaptureForm : Form
{
    ...
    private void InitializeComponent()
    {
        ...
        } else
        {
            ...
            } else
            {
                ...
                {
                    int margin = 100;
                    Size canvasWindowSize = new(Canvas.Width + SystemInformation.BorderSize.Width * 2 + margin,
    ...
    internal void InitBackground(Bitmap canvas, bool centerCanvas = true)
    {
        ...
        if (IsEditorMode)
        {
            ...
            using (Bitmap background = new(Canvas.Width, Canvas.Height))
            using (Graphics g = Graphics.FromImage(background))
            {
                Rectangle sourceRect = new(0, 0, Canvas.Width, Canvas.Height);
    ...
    {
        SizeF panLimitSize = new(Math.Min(ClientArea.Width * 0.25f, CanvasRectangle.Width),
            Math.Min(ClientArea.Height * 0.25f, CanvasRectangle.Height));

        RectangleF limitRectangle = new(ClientArea.X + panLimitSize.Width, ClientArea.Y + panLimitSize.Height,
    ...
    {
        PointF clientCenter = new(ClientArea.Width / 2f, ClientArea.Height / 2f);
    ...
    private new void Update()
    {
        ...
        {
            Vector2 offset = new(CanvasCenterOffset.X + InputManager.MouseVelocity.X, CanvasCenterOffset.Y + InputManager.MouseVelocity.Y);
            PanToOffset(offset);
        }
    ...
    {
        using GraphicsQualityManager quality = new(g, false);
    ...
    private void DrawShapes(Graphics g)
    {
        ...
        if (areas.Count > 0)
        {
            ...
            {
                using Region region = new(regionDrawPath);
                g.Clip = region;
        ...
        if (ShapeManager.IsCurrentHoverShapeValid)
        {
            if (Options.EnableAnimations)
            {
                ...
                {
                    regionAnimation.FromRectangle = regionAnimation.CurrentRectangle.Width > 2 && regionAnimation.CurrentRectangle.Height > 2
                        ? regionAnimation.CurrentRectangle
                        : ShapeManager.PreviousHoverRectangle;
            ...
            using GraphicsPath hoverDrawPath = new() { FillMode = FillMode.Winding };
    ...
    {
        Point textPosition = new(offset, offset);
    ...
    internal void DrawAreaText(Graphics g, string text, RectangleF area)
    {
        ...
        PointF textPos = area.Y - offset - textSize.Height - backgroundPadding * 2 < ClientArea.Y
            ? new PointF(area.X + offset + backgroundPadding, area.Y + offset + backgroundPadding)
            : new PointF(area.X + backgroundPadding, area.Y - offset - backgroundPadding - textSize.Height);
        ...
        RectangleF backgroundRect = new(textPos.X - backgroundPadding, textPos.Y - backgroundPadding, textSize.Width + backgroundPadding * 2, textSize.Height + backgroundPadding * 2);
    ...
    private void DrawTextAnimation(Graphics g, TextAnimation textAnimation)
    {
        ...
        Rectangle textRectangle = new(textAnimation.Position.X, textAnimation.Position.Y, textSize.Width, textSize.Height);
    ...
    private void DrawTextAnimation(Graphics g, TextAnimation textAnimation, Rectangle textRectangle, int padding)
    {
        ...
        using Pen outerBorderPen = new(Color.FromArgb((int)(textAnimation.Opacity * 200), textOuterBorderColor));
        using Pen innerBorderPen = new(Color.FromArgb((int)(textAnimation.Opacity * 200), textInnerBorderColor));
    ...
    private void DrawBottomTipAnimation(Graphics g, TextAnimation textAnimation)
    {
        ...
        textSize.Height += padding * 2;
        int margin = 20;
        Rectangle textRectangle = new(ClientArea.Width / 2 - textSize.Width / 2, ClientArea.Height - textSize.Height - margin, textSize.Width, textSize.Height);
        DrawTextAnimation(g, textAnimation, textRectangle, padding);
    }
    ...
    internal string GetAreaText(RectangleF rect)
    {
        ...
        {
            PointF endLocation = new(rect.Right - 1, rect.Bottom - 1);
    ...
    private void DrawCrosshair(Graphics g)
    {
        ...
        PointF left = new(mousePos.X - offset, mousePos.Y), left2 = new(0, mousePos.Y);
        PointF right = new(mousePos.X + offset, mousePos.Y), right2 = new((ClientArea.Width - 1) / ZoomFactor, mousePos.Y);
        PointF top = new(mousePos.X, mousePos.Y - offset), top2 = new(mousePos.X, 0);
        PointF bottom = new(mousePos.X, mousePos.Y + offset), bottom2 = new(mousePos.X, (ClientArea.Height - 1) / ZoomFactor);
    ...
    private void DrawCursorGraphics(Graphics g)
    {
        ...
        if (Options.ShowMagnifier)
        {
            ...
            {
                using GraphicsQualityManager quality = new(g, true);
                using TextureBrush brush = new(magnifier);
        ...
        if (Options.ShowInfo)
        {
            if (Mode == RegionCaptureMode.ScreenColorPicker)
            {
                ...
                Point padding = new(infoTextPadding + colorBoxExtraWidth, infoTextPadding);

                Rectangle colorRect = new(infoTextRect.X + colorBoxOffset, infoTextRect.Y + colorBoxOffset, colorBoxSize, colorBoxSize);
            ...
            {
                infoTextRect.Location = new Point(x + totalSize.Width / 2 - infoTextRect.Width / 2, y + infoTextPosition);
                Point padding = new(infoTextPadding, infoTextPadding);
    ...
    private Bitmap Magnifier(Image img, PointF position, int horizontalPixelCount, int verticalPixelCount, int pixelSize)
    {
        ...
        Bitmap bmp = new(width - 1, height - 1);
        ...
        using (Graphics g = Graphics.FromImage(bmp))
        {
            ...
            using (SolidBrush crosshairBrush = new(Color.FromArgb(125, Color.LightBlue)))
            {
            ...
            using (Pen pen = new(Color.FromArgb(75, Color.Black)))
            {
    ...
    public Bitmap GetResultImage()
    {
        ...
        {
            GraphicsPath gp = Result == RegionResult.LastRegion ? LastRegionFillPath : regionFillPath;
            if (gp != null)
            {
