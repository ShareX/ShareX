...
using ShareX.ScreenCaptureLib.Shapes.Region;
...
internal partial class ShapeManager : IDisposable
{
    ...
    public RectangleF CurrentRectangle
    {
        get
        {
            return CurrentShape != null ? CurrentShape.Rectangle : RectangleF.Empty;
        }
    }

    public PointF CurrentDPI = new(96f, 96f);
    ...
    public ShapeManager(RegionCaptureForm form)
    {
        ...
        CurrentTool = form.Mode == RegionCaptureMode.Annotation
            ? Options.LastRegionTool
            : form.IsEditorMode ? Options.LastEditorTool : ShapeType.RegionRectangle;
    ...
    private void SwapShapeType()
    {
        if (Form.Mode == RegionCaptureMode.Annotation)
        {
            CurrentTool = IsCurrentShapeTypeRegion ? Options.LastAnnotationTool : Options.LastRegionTool;
        }
    }
    ...
    public PointF SnapPosition(PointF posOnClick, PointF posCurrent)
    {
        ...
        Vector2 vector = new(currentSize.Width, currentSize.Height);
    ...
    {
        return Windows != null ? Windows.FirstOrDefault(x => x.Rectangle.Contains(InputManager.MousePosition)) : null;
    }
    ...
    private void PasteFromClipboard(bool insertMousePosition)
    {
        ...
        } else if (ClipboardHelpers.ContainsText())
        {
            ...
            {
                PointF pos = insertMousePosition ? Form.ScaledClientMousePosition : (PointF)Form.ClientArea.Center();
                CurrentTool = ShapeType.DrawingTextBackground;
    ...
    public Bitmap CropImage(RectangleF rect, bool onlyIfSizeDifferent = false)
    {
        ...
        return cropRect.IsValid() && (!onlyIfSizeDifferent || cropRect.Size != Form.Canvas.Size)
            ? ImageHelpers.CropBitmap(Form.Canvas, cropRect)
            : null;
    }
    ...
    private void InsertImage(Image img)
    {
        if (img != null)
        {
            ...
            using (ImageInsertForm imageInsertForm = new())
            {
    ...
    private void ChangeImageSize()
    {
        ...
        using (ImageSizeForm imageSizeForm = new(oldSize, Options.ImageEditorResizeInterpolationMode))
        {
    ...
    private void ChangeCanvasSize()
    {
        ...
        using (CanvasSizeForm canvasSizeForm = new(Padding.Empty, Options.EditorCanvasColor))
        {
    ...
    public void AutoResizeCanvas()
    {
        ...
        {
            Padding margin = new((int)Math.Round(Math.Max(0, canvas.X - combinedImageRectangle.X)), (int)Math.Round(Math.Max(0, canvas.Y - combinedImageRectangle.Y)),
    ...
    {
        Rectangle source = new(0, 0, Form.Canvas.Width, Form.Canvas.Height);
        RectangleF crop;
    ...
    private void AddImageEffects()
    {
        ...
        using (ImageEffectsForm imageEffectsForm = new(Form.Canvas, Options.ImageEffectPresets, Options.SelectedImageEffectPreset))
        {
