...
{
    public class EraserDrawingShape : BaseDrawingShape
    {
        ...
        protected void DrawFreehand(Graphics g)
        {
            if (Shadow)
            {
                DrawFreehand(g, BorderSize, positions.Select(x => x.Add(ShadowOffset)).ToArray());
            }

            DrawFreehand(g, BorderSize, positions.ToArray());
        }

        protected void DrawFreehand(Graphics g, int borderSize, Point[] points)
        {
            var color = Color.White;
            if (points.Length > 0 && borderSize > 0)
            {
                ...
                {
                    using (Pen pen = new Pen(color, borderSize) { StartCap = LineCap.Round, EndCap = LineCap.Round, LineJoin = LineJoin.Round })
                    {
