...
public class FreehandDrawingShape : BaseDrawingShape
{
    ...
    public PointF LastPosition
    {
        get
        {
            return positions.Count > 0 ? positions[positions.Count - 1] : (PointF)Point.Empty;
        }
        set
        {
    ...
    protected List<PointF> positions = new();
    ...
    protected void DrawFreehand(Graphics g, Color borderColor, int borderSize, BorderStyle borderStyle, PointF[] points)
    {
        ...
        {
            g.SmoothingMode = Manager.IsRenderingOutput ? SmoothingMode.HighQuality : SmoothingMode.HighSpeed;

            if (points.Length == 1)
            {
                using Brush brush = new SolidBrush(borderColor);
                Rectangle rect = new((int)(points[0].X - borderSize / 2f), (int)(points[0].Y - borderSize / 2f), borderSize, borderSize);
