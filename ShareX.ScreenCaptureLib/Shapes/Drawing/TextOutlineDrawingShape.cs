    ...
    protected void DrawTextWithOutline(Graphics g, string text, TextDrawingOptions options, Color textColor, Color borderColor, int borderSize, RectangleF rect)
    {
        if (!string.IsNullOrEmpty(text) && rect.Width > 10 && rect.Height > 10)
        {
            ...
            if (Shadow)
            {
                ...
                if (IsBorderVisible)
                {
                    using Pen shadowPen = new(ShadowColor, borderSize) { LineJoin = LineJoin.Round };
            ...
            try
            {
                textBrush = TextOptions.Gradient
                    ? new LinearGradientBrush(pathRect.Round().Offset(1), textColor, TextOptions.Color2, TextOptions.GradientMode)
