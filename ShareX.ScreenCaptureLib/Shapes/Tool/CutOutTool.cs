...
public class CutOutTool : BaseTool
{
    ...
    public override bool IsValidShape
    {
        get
        {
            return !IsHorizontalTrim && !IsVerticalTrim
                ? false
                : IsHorizontalTrim && Rectangle.Left <= Manager.Form.CanvasRectangle.Left && Rectangle.Right >= Manager.Form.CanvasRectangle.Right
                ? false
                : !IsVerticalTrim || Rectangle.Top > Manager.Form.CanvasRectangle.Top || Rectangle.Bottom < Manager.Form.CanvasRectangle.Bottom;
        }
    }
    ...
    public RectangleF CutOutRectangle
    {
        get
        {
            return IsHorizontalTrim
                ? new RectangleF(Rectangle.X, Manager.Form.CanvasRectangle.Y, Rectangle.Width, Manager.Form.CanvasRectangle.Height)
                : IsVerticalTrim
                ? new RectangleF(Manager.Form.CanvasRectangle.X, Rectangle.Y, Manager.Form.CanvasRectangle.Width, Rectangle.Height)
                : RectangleF.Empty;
        }
    }

    private ImageEditorButton confirmButton, cancelButton;
    private Size buttonSize = new(80, 40);
