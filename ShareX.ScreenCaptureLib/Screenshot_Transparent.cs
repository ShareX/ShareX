...
public partial class Screenshot
{
    ...
    private Bitmap CreateTransparentImage(Bitmap whiteBackground, Bitmap blackBackground)
    {
        ...
        {
            Bitmap result = new(whiteBackground.Width, whiteBackground.Height, PixelFormat.Format32bppArgb);

            using (UnsafeBitmap whiteBitmap = new(whiteBackground, true, ImageLockMode.ReadOnly))
            using (UnsafeBitmap blackBitmap = new(blackBackground, true, ImageLockMode.ReadOnly))
            using (UnsafeBitmap resultBitmap = new(result, true, ImageLockMode.WriteOnly))
            {
    ...
    private void TrimShadow(Bitmap bitmap)
    {
        ...
        using UnsafeBitmap unsafeBitmap = new(bitmap, true);
