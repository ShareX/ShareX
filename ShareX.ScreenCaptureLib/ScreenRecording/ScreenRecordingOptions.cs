...
public class ScreenRecordingOptions
{
    ...
    public string GetFFmpegCommands()
    {
        ...
        commands = !IsLossless && FFmpeg.UseCustomCommands && !string.IsNullOrEmpty(FFmpeg.CustomCommands)
            ? FFmpeg.CustomCommands.
        ...
        if (FFmpeg.IsVideoSourceSelected)
        {
            if (IsLossless || FFmpeg.VideoCodec != FFmpegVideoCodec.apng)
            {
                string videoCodec = IsLossless
                    ? FFmpegVideoCodec.libx264.ToString()
                    : FFmpeg.VideoCodec == FFmpegVideoCodec.libvpx_vp9 ? "libvpx-vp9" : FFmpeg.VideoCodec.ToString();
