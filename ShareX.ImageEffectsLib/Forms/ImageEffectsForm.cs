...
using ShareX.ImageEffectsLib.Adjustments;
using ShareX.ImageEffectsLib.Drawings;
using ShareX.ImageEffectsLib.Filters;
...
public partial class ImageEffectsForm : Form
{
    ...
    {
        ToolStripMenuItem tsmiParent = new(groupName);
    ...
    private void LoadSettings()
    {
        if (Presets != null && Presets.Count > 0)
        {
            ...
            {
                ListViewItem lvi = new(preset.ToString());
    ...
    private void GeneratePreviewImage(int padding)
    {
        if (pbResult.ClientSize.Width > 0 && pbResult.ClientSize.Height > 0)
        {
            ...
            Color backgroundColor = ShareXResources.UseCustomTheme ? ShareXResources.Theme.BackgroundColor : Color.DarkGray;
    ...
    private Bitmap ApplyEffects()
    {
        ...
        return preset != null ? preset.ApplyEffects(PreviewImage) : null;
    }
    ...
    {
        ListViewItem lvi = new(imageEffect.ToString());
    ...
    private void ImageEffectsForm_DragEnter(object sender, DragEventArgs e)
    {
        e.Effect = e.Data.GetDataPresent(DataFormats.FileDrop, false)
            ? e.Data.GetData(DataFormats.FileDrop, false) is string[] files && files.Any(x => !string.IsNullOrEmpty(x) && x.EndsWith(".sxie"))
    ...
    private void btnPackager_Click(object sender, EventArgs e)
    {
        ...
        if (preset != null)
        {
            ...
            } else
            {
                ...
                {
                    using ImageEffectPackagerForm packagerForm = new(json, preset.Name,
    ...
    {
        e.Effect = e.Data.GetDataPresent(DataFormats.FileDrop, false) || e.Data.GetDataPresent(DataFormats.Bitmap, false)
            ? DragDropEffects.Copy
            : DragDropEffects.None;
    }
