...
public class DrawTextEx : ImageEffect
{
    ...
    public override Bitmap Apply(Bitmap bmp)
    {
        ...
        NameParser parser = new(NameParserType.Text);
        ...
        using (GraphicsPath gp = new())
        {
            ...
            Rectangle textRectangle = new(textPosition, textSize);
            ...
            if (Shadow && (!ShadowUseGradient && ShadowColor.A > 0 || ShadowUseGradient && ShadowGradient.IsVisible))
            {
                ...
                if (Outline && OutlineSize > 0)
                {
                    if (ShadowUseGradient)
                    {
                        ...
                        using Pen textShadowPen = new(textShadowBrush, OutlineSize) { LineJoin = LineJoin.Round };
                        g.DrawPath(textShadowPen, gp);
                    } else
                    {
                        using Pen textShadowPen = new(ShadowColor, OutlineSize) { LineJoin = LineJoin.Round };
                        g.DrawPath(textShadowPen, gp);
                    }
                } else
                {
            ...
            if (Outline && OutlineSize > 0)
            {
                if (OutlineUseGradient)
                {
                    if (OutlineGradient.IsVisible)
                    {
                        ...
                        using Pen textOutlinePen = new(textOutlineBrush, OutlineSize) { LineJoin = LineJoin.Round };
                        g.DrawPath(textOutlinePen, gp);
                    }
                } else if (OutlineColor.A > 0)
                {
                    using Pen textOutlinePen = new(OutlineColor, OutlineSize) { LineJoin = LineJoin.Round };
                    g.DrawPath(textOutlinePen, gp);
                }
            }
    ...
    {
        return !string.IsNullOrEmpty(Text) ? Text.Truncate(20, "...") : null;
    }
}