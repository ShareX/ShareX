...
public class TaskSettings
{
    ...
    public TaskSettingsImage ImageSettingsReference
        => UseDefaultImageSettings ? Program.DefaultTaskSettings.ImageSettings : TaskSettingsReference.ImageSettings;

    public bool UseDefaultCaptureSettings = true;
    public TaskSettingsCapture CaptureSettings = new();

    [JsonIgnore]
    public TaskSettingsCapture CaptureSettingsReference
        => UseDefaultCaptureSettings ? Program.DefaultTaskSettings.CaptureSettings : TaskSettingsReference.CaptureSettings;

    public bool UseDefaultUploadSettings = true;
    public TaskSettingsUpload UploadSettings = new();

    public bool UseDefaultActions = true;
    public List<ExternalProgram> ExternalPrograms = new();

    public bool UseDefaultToolsSettings = true;
    public TaskSettingsTools ToolsSettings = new();

    [JsonIgnore]
    public TaskSettingsTools ToolsSettingsReference => UseDefaultToolsSettings ? Program.DefaultTaskSettings.ToolsSettings : TaskSettingsReference.ToolsSettings;

    public bool UseDefaultAdvancedSettings = true;
    public TaskSettingsAdvanced AdvancedSettings = new();

    public bool WatchFolderEnabled = false;
    public List<WatchFolderSettings> WatchFolderList = new();

    public override string ToString() => !string.IsNullOrEmpty(Description) ? Description : Job.GetLocalizedDescription();

    public bool IsUsingDefaultSettings
        => UseDefaultAfterCaptureJob && UseDefaultAfterUploadJob
        && UseDefaultDestinations && !OverrideFTP
        && !OverrideCustomUploader && !OverrideScreenshotsFolder
        && UseDefaultGeneralSettings && UseDefaultImageSettings
        && UseDefaultCaptureSettings && UseDefaultUploadSettings
        && UseDefaultActions && UseDefaultToolsSettings
        && UseDefaultAdvancedSettings && !WatchFolderEnabled;

    public static TaskSettings GetDefaultTaskSettings()
    {
        TaskSettings taskSettings = new();
...
public class TaskSettingsGeneral
{
    ...
    public Size ToastWindowSize = new(400, 300);
...
public class TaskSettingsCapture
{
    ...
    public Rectangle CaptureCustomRegion = new(0, 0, 0, 0);
    ...
    public ScrollingCaptureOptions ScrollingCaptureOptions = new();

    #endregion Capture / Scrolling capture

    #region Capture / OCR

    public OCROptions OCROptions = new();
...
public class TaskSettingsUpload
{
    ...
    public List<UploaderFilter> UploaderFilters = new();
