...
internal static class SettingManager
{
    ...
    private static string ApplicationConfigFilePath
    {
        get
        {
            return Program.Sandbox ? null : Path.Combine(Program.PersonalFolder, ApplicationConfigFileName);
        }
    }
    ...
    private static string UploadersConfigFilePath
    {
        get
        {
            ...
            string uploadersConfigFolder = Settings != null && !string.IsNullOrEmpty(Settings.CustomUploadersConfigPath)
                ? FileHelpers.ExpandFolderVariables(Settings.CustomUploadersConfigPath)
                : Program.PersonalFolder;
    ...
    private static string HotkeysConfigFilePath
    {
        get
        {
            ...
            string hotkeysConfigFolder = Settings != null && !string.IsNullOrEmpty(Settings.CustomHotkeysConfigPath)
                ? FileHelpers.ExpandFolderVariables(Settings.CustomHotkeysConfigPath)
                : Program.PersonalFolder;
    ...
    {
        string message = e is UnauthorizedAccessException || e is FileNotFoundException
    ...
    private static void ApplicationConfigBackwardCompatibilityTasks()
    {
        ...
        if (Settings.IsUpgradeFrom("14.1.1"))
        {
            if (Helpers.IsDefaultSettings(Settings.Themes, ShareXTheme.GetDefaultThemes(), (x, y) => x.Name == y.Name))
            {
                ...
                Settings.SelectedTheme = index >= 0 ? index : 0;
            }
        }
    ...
    private static void MigrateHistoryFile()
    {
        if (File.Exists(Program.HistoryFilePathOld))
        {
            if (!File.Exists(Program.HistoryFilePath))
            {
                ...
                HistoryManagerXML historyManagerXML = new(Program.HistoryFilePathOld);
                List<HistoryItem> historyItems = historyManagerXML.GetHistoryItems();

                if (historyItems.Count > 0)
                {
                    HistoryManagerJSON historyManagerJSON = new(Program.HistoryFilePath);
