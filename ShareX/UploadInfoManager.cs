...
public class UploadInfoManager
{
    ...
    public UploadInfoStatus SelectedItem
    {
        get
        {
            return IsItemSelected ? SelectedItems[0] : null;
        }
    }
    ...
    {
        SelectedItems = tasks != null && tasks.Count() > 0
            ? tasks.Where(x => x != null && x.Info != null).Select(x => new UploadInfoStatus(x)).ToArray()
            : null;
    }
