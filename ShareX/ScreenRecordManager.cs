...
public static class ScreenRecordManager
{
    ...
    private static void StartRecording(ScreenRecordOutput outputType, TaskSettings taskSettings, ScreenRecordStartMethod startMethod = ScreenRecordStartMethod.Region)
    {
        ...
        int fps = taskSettings.CaptureSettings.FFmpegOptions.VideoCodec == FFmpegVideoCodec.gif
        ...
        switch (startMethod)
        {
            ...
            case ScreenRecordStartMethod.ActiveWindow:
                captureRectangle = taskSettings.CaptureSettings.CaptureClientArea
                    ? HelpersLib.Helpers.CaptureHelpers.GetActiveWindowClientRectangle()
                    : HelpersLib.Helpers.CaptureHelpers.GetActiveWindowRectangle();

                IntPtr handle = NativeMethods.GetForegroundWindow();
                WindowInfo activeWindowInfo = new(handle);
        ...
        Task.Run(() =>
        {
            try
            {
                string extension = taskSettings.CaptureSettings.ScreenRecordTwoPassEncoding ? "mp4" : taskSettings.CaptureSettings.FFmpegOptions.Extension;
                ...
                while (!abortRequested && (recordForm.Status == ScreenRecordingStatus.Waiting || recordForm.Status == ScreenRecordingStatus.Paused))
                {
                    ...
                    {
                        using FFmpegCLIManager ffmpeg = new(taskSettings.CaptureSettings.FFmpegOptions.FFmpegPath);
