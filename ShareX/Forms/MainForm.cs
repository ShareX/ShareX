...
public partial class MainForm : HotkeyForm
{
    ...
    private ImageFilesCache actionsMenuIconCache = new();
    ...
    private void InitializeControls()
    {
        ...
        {
            ToolStripMenuItem tsmi = new(urlShortener.GetLocalizedDescription());
        ...
        {
            ToolStripMenuItem tsmi = new(urlSharingService.GetLocalizedDescription());
    ...
    private void UpdateWorkflowsMenu()
    {
        ...
        ToolStripMenuItem tsmi = new(Resources.MainForm_UpdateWorkflowsMenu_You_can_add_workflows_from_hotkey_settings___);
    ...
    {
        ToolStripMenuItem tsmi = new(hotkeySetting.TaskSettings.ToString().Replace("&", "&&"));
    ...
    private void AddEnumItems<T>(Action<T> selectedEnum, params ToolStripDropDownItem[] parents) where T : Enum
    {
        ...
        foreach (ToolStripDropDownItem parent in parents)
        {
            for (int i = 0; i < enums.Length; i++)
            {
                ...
                ToolStripMenuItem tsmi = new(currentEnum.GetLocalizedDescription());
    ...
    private void UpdateImageEffectsMenu(ToolStripDropDownItem parent)
    {
        ...
        if (count > 0)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    ToolStripMenuItem tsmi = new(effectPreset.ToString());
        ...
        if (uim.IsItemSelected)
        {
            ...
            } else
            {
                ...
                if (Program.Settings.ClipboardContentFormats != null && Program.Settings.ClipboardContentFormats.Count > 0)
                {
                    ...
                    {
                        ToolStripMenuItem tsmiClipboardFormat = new(cf.Description);
    ...
    {
        return lvUploads.SelectedItems.Count > 0
            ? [.. lvUploads.SelectedItems.Cast<ListViewItem>().Select(x => x.Tag as WorkerTask).Where(x => x != null)]
            : null;
    }
    ...
    private static async Task PrepareCaptureMenuAsync(ToolStripMenuItem tsmiWindow, EventHandler handlerWindow, ToolStripMenuItem tsmiMonitor, EventHandler handlerMonitor)
    {
        tsmiWindow.DropDownItems.Clear();
        WindowsList windowsList = new();
        ...
        if (windows != null && windows.Count > 0)
        {
            ...
            foreach (WindowInfo window in windows)
            {
                try
                {
                    ...
                    ToolStripMenuItem tsmi = new(title);
                    tsmi.Tag = window;
    ...
    private void tsbHotkeySettings_Click(object sender, EventArgs e)
    {
        if (Program.HotkeyManager != null)
        {
            using (HotkeySettingsForm hotkeySettingsForm = new(Program.HotkeyManager))
            {
