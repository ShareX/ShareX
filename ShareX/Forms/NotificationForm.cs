...
public class NotificationForm : LayeredForm
{
    ...
    {
        Rectangle rect = new(0, 0, buffer.Width, buffer.Height);
        ...
        if (Config.Image != null)
        {
            ...
            {
                Rectangle textRect = new(0, 0, rect.Width, 40);

                using (SolidBrush brush = new(Color.FromArgb(100, 0, 0, 0)))
                {
        ...
        } else if (!string.IsNullOrEmpty(Config.Text))
        {
            ...
            if (!string.IsNullOrEmpty(Config.Title))
            {
                Rectangle titleRect = new(Config.TextPadding, Config.TextPadding, titleRenderSize.Width + 2, titleRenderSize.Height + 2);
        ...
        using Pen borderPen = new(Config.BorderColor);
    ...
    private void NotificationForm_MouseMove(object sender, MouseEventArgs e)
    {
        if (isMouseDragging)
        {
            ...
            Rectangle dragThresholdRectangle = new(dragStart.X - dragThreshold, dragStart.Y - dragThreshold, dragThreshold * 2, dragThreshold * 2);
