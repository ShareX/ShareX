...
public partial class ApplicationSettingsForm : Form
{
    ...
    private void InitializeControls()
    {
        ...
        {
            ToolStripMenuItem tsmi = new(language.GetLocalizedDescription());
    ...
    private void UpdatePersonalFolderPathPreview()
    {
        try
        {
            ...
            personalPath = string.IsNullOrEmpty(personalPath)
                ? Program.Portable ? Program.PortablePersonalFolder : Program.DefaultPersonalFolder
                : FileHelpers.GetAbsolutePath(personalPath);
    ...
    private void CbThemes_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (ready)
        {
            ...
            pgTheme.SelectedObject = cbThemes.SelectedItem != null ? cbThemes.SelectedItem : null;
    ...
    private void BtnThemeRemove_Click(object sender, EventArgs e)
    {
        ...
        if (index > -1)
        {
            ...
            index = Program.Settings.Themes.Count > 0 ? 0 : -1;
            Program.Settings.SelectedTheme = index;
            cbThemes.SelectedIndex = index;
    ...
    {
        ListViewItem lvi = new(cf.Description ?? "");
        lvi.Tag = cf;
    ...
    private void ClipboardFormatsEditSelected()
    {
        if (lvClipboardFormats.SelectedItems.Count > 0)
        {
            ...
            using ClipboardFormatForm form = new(cf);
    ...
    {
        using ClipboardFormatForm form = new();
        if (form.ShowDialog() == DialogResult.OK)
        {
    ...
    private void btnShowImagePrintSettings_Click(object sender, EventArgs e)
    {
        ...
        using PrintForm printForm = new(testImage, Program.Settings.PrintSettings, true);
