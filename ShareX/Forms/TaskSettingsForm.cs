...
public partial class TaskSettingsForm : Form
{
    ...
    {
        Text = IsDefault
            ? "ShareX - " + Resources.TaskSettingsForm_UpdateWindowTitle_Task_settings
            : "ShareX - " + string.Format(Resources.TaskSettingsForm_UpdateWindowTitle_Task_settings_for__0_, TaskSettings);
    }
    ...
    private void btnRegionCaptureSnapSizesDialogAdd_Click(object sender, EventArgs e)
    {
        ...
        SnapSize size = new((int)nudRegionCaptureSnapSizesWidth.Value, (int)nudRegionCaptureSnapSizesHeight.Value);
    ...
    private void AddUploaderFilterToList(UploaderFilter filter)
    {
        if (filter != null)
        {
            ListViewItem lvi = new(filter.Uploader);
    ...
    {
        ListViewItem lvi = new(fileAction.Name ?? "");
        lvi.Tag = fileAction;
    ...
    private void btnActionsEdit_Click(object sender, EventArgs e)
    {
        if (lvActions.SelectedItems.Count > 0)
        {
            ...
            using ActionsForm form = new(fileAction);
            if (form.ShowDialog() == DialogResult.OK)
            {
    ...
    private void WatchFolderAdd(WatchFolderSettings watchFolderSetting)
    {
        if (Program.WatchFolderManager != null && watchFolderSetting != null)
        {
            ...
            ListViewItem lvi = new(watchFolderSetting.FolderPath ?? "");
    ...
    private void WatchFolderEditSelected()
    {
        if (lvWatchFolderList.SelectedItems.Count > 0)
        {
            ...
            using WatchFolderForm form = new(watchFolder);
            if (form.ShowDialog() == DialogResult.OK)
            {
