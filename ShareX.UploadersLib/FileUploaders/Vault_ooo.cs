...
public sealed class Vault_ooo : FileUploader
{
    ...
    private static readonly DateTime ORIGIN_TIME = new(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc);
    ...
    public override UploadResult Upload(Stream stream, string fileName)
    {
        ...
        using (MemoryStream ms = new()) // Encrypting file name
        {
            ...
            encryptedFileName = HelpersLib.Helpers.Helpers.BytesToHex(ms.ToArray());
        }
        ...
        Dictionary<string, long> postRequestJson = new();
    ...
    private static Vault_oooCryptoData DeriveCryptoData(byte[] key)
    {
        ...
        Rfc2898DeriveBytes rfcDeriver = new(key, salt, PBKDF2_ITERATIONS, HashAlgorithmName.SHA256);
    ...
    private static byte[] EncryptBytes(Vault_oooCryptoData crypto, byte[] bytes)
    {
        ...
        using MemoryStream ms = new();
        using CryptoStream cs = new(ms, aes.CreateEncryptor(crypto.Key, crypto.IV), CryptoStreamMode.Write);
