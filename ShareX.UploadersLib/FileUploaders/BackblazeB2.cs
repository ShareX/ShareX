...
public sealed class BackblazeB2 : ImageUploader
{
    ...
    public override UploadResult Upload(Stream stream, string fileName)
    {
        ...
        for (int tries = 1; tries <= maxTries; tries++)
        {
            ...
            HashSet<string> expiredTokenCodes = new(new List<string> { "expired_auth_token", "bad_auth_token" });
    ...
    private string B2ApiGetBucketId(B2Authorization auth, string bucketName, out string error)
    {
        ...
        Dictionary<string, string> reqBody = new()
        {
    ...
    {
        NameValueCollection headers = new() { ["Authorization"] = auth.authorizationToken };

        Dictionary<string, string> reqBody = new() { ["bucketId"] = bucketId };
    ...
    private B2UploadResult B2ApiUploadFile(B2UploadUrl b2UploadUrl, string destinationPath, Stream file)
    {
        ...
        ContentDisposition contentDisposition = new("inline") { FileName = URLHelpers.GetFileName(destinationPath) };
