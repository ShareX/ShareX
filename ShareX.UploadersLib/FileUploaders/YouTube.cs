...
public sealed class YouTube : FileUploader, IOAuth2
{
    ...
    public override UploadResult Upload(Stream stream, string fileName)
    {
        ...
        if (ShowDialog)
        {
            using YouTubeVideoOptionsForm form = new(title, description, visibility);
        ...
        YouTubeVideoUpload uploadVideo = new()
        {
        ...
        if (!string.IsNullOrEmpty(result.Response))
        {
            ...
            {
                result.URL = UseShortenedLink ? $"https://youtu.be/{responseVideo.id}" : $"https://www.youtube.com/watch?v={responseVideo.id}";
