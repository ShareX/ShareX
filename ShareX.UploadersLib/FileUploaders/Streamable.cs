...
public class Streamable : FileUploader
{
    ...
    private void TranscodeFile(UploadResult result)
    {
        ...
        {
            ProgressManager progress = new(100);
            ...
            while (!StopUploadRequested)
            {
                ...
                } else if (response.status == 2)
                {
                    ...
                    result.URL = UseDirectURL && response.files != null && response.files.mp4 != null && !string.IsNullOrEmpty(response.files.mp4.url)
                        ? URLHelpers.ForcePrefix(response.files.mp4.url)
                        : URLHelpers.ForcePrefix(response.url);
