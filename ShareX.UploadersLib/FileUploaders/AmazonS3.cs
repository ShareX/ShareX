...
public sealed class AmazonS3 : FileUploader
{
    ...
    public override UploadResult Upload(Stream stream, string fileName)
    {
        ...
        string hashedPayload = Settings.SignedPayload
            ? HelpersLib.Helpers.Helpers.BytesToHex(HelpersLib.Helpers.Helpers.ComputeSHA256(stream))
            : "UNSIGNED-PAYLOAD";
        ...
        string stringToSign = algorithm + "\n" +
            timeStamp + "\n" +
            scope + "\n" +
            HelpersLib.Helpers.Helpers.BytesToHex(HelpersLib.Helpers.Helpers.ComputeSHA256(canonicalRequest));

        byte[] dateKey = HelpersLib.Helpers.Helpers  .ComputeHMACSHA256(credentialDate, "AWS4" + Settings.SecretAccessKey);
        byte[] dateRegionKey = HelpersLib.Helpers.Helpers.ComputeHMACSHA256(region, dateKey);
        byte[] dateRegionServiceKey = HelpersLib.Helpers.Helpers.ComputeHMACSHA256("s3", dateRegionKey);
        byte[] signingKey = HelpersLib.Helpers.Helpers.ComputeHMACSHA256("aws4_request", dateRegionServiceKey);

        string signature = HelpersLib.Helpers.Helpers.BytesToHex(HelpersLib.Helpers.Helpers.ComputeHMACSHA256(stringToSign, signingKey));
    ...
    private string GetRegion()
    {
        ...
        return separatorIndex == -1 ? DefaultRegion : serviceAndRegion.Substring(separatorIndex + 1);
    }
