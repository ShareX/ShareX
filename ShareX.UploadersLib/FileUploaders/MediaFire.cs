...
public class MediaFireFileUploaderService : FileUploaderService
{
    ...
    {
        return new MediaFire(APIKeys.APIKeys.MediaFireAppId, APIKeys.APIKeys.MediaFireApiKey, config.MediaFireUsername, config.MediaFirePassword)
        {
...
public sealed class MediaFire : FileUploader
{
    ...
    private string SimpleUpload(Stream stream, string fileName)
    {
        ...
        EnsureSuccess(resp);
        return resp.doupload.result != 0 || resp.doupload.key == null ? throw new IOException("Invalid response") : resp.doupload.key;
    }
    ...
    {
        return args != null && args.Count > 0 ? url + "?" + string.Join("&", args.Select(x => x.Key + "=" + x.Value).ToArray()) : url;
    }
