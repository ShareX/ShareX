...
public class CustomFileUploaderService : FileUploaderService
{
    ...
    public override GenericUploader CreateUploader(UploadersConfig config, TaskReferenceHelper taskInfo)
    {
        ...
        return customUploader != null ? new CustomFileUploader(customUploader) : (GenericUploader)null;
    }
}
    ...
    public override UploadResult Upload(Stream stream, string fileName)
    {
        ...
        } else
        {
            result.Response = uploader.Body == CustomUploaderBody.Binary
                ? SendRequest(uploader.RequestMethod, uploader.GetRequestURL(input), stream, MimeTypes.GetMimeTypeFromFileName(fileName), null,
