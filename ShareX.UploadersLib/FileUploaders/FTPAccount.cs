...
public class FTPAccount : ICloneable
{
        ...
        } else
        {
            ...
            if (httpHomeUri.Query.EndsWith("="))
            {
                ...
                httpHomeUri.Query = HttpHomePathAutoAddSubFolderPath
                    ? URLHelpers.CombineURL(httpHomeUri.Query.Substring(1), subFolderPath, fileName)
                    : httpHomeUri.Query.Substring(1) + fileName;
            } else
            {
                if (HttpHomePathAutoAddSubFolderPath)
                {
    ...
    {
        return string.IsNullOrEmpty(FTPAddress) ? "" : URLHelpers.CombineURL(FTPAddress, GetSubFolderPath(fileName, NameParserType.FilePath));
    }
