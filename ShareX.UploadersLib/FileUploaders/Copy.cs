...
public sealed class Copy : FileUploader, IOAuth
{
    ...
    private readonly NameValueCollection APIHeaders = new() { { "X-Api-Version", APIVersion } };
    ...
    public string CreatePublicURL(string path, CopyURLType urlType = CopyURLType.Default)
    {
        ...
        CopyLinkRequest publicLink = new();
    ...
    {
        return !string.IsNullOrEmpty(uploadPath) ? uploadPath.Trim().Replace('\\', '/').Trim('/') + "/" : "";
    }
}
