...
public sealed class FTP : FileUploader, IDisposable
{
    ...
    {
        using FileStream stream = new(localPath, FileMode.Open);
        UploadData(stream, remotePath);
    }
    ...
    {
        using MemoryStream stream = new(Encoding.UTF8.GetBytes(text), false);
        UploadData(stream, remotePath);
    }
    ...
    {
        return Connect() ? client.DirectoryExists(remotePath) : false;
    }
