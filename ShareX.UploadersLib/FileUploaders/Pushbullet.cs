...
public sealed class Pushbullet : FileUploader
{
    ...
    public UploadResult PushFile(Stream stream, string fileName)
    {
        ...
        Dictionary<string, string> pushArgs, upArgs = new();
    ...
    private string Push(string pushType, string valueType, string value, string title)
    {
        ...
        return push != null ? wwwPushesURL + "?push_iden=" + push.iden : null;
    }
    ...
    {
        return string.IsNullOrEmpty(Config.UserAPIKey)
            ? throw new Exception("Missing API key.")
            : Config.CurrentDevice == null
            ? throw new Exception("No device set to push to.")
            : string.IsNullOrEmpty(Config.CurrentDevice.Key) ? throw new Exception("Missing device key.") : PushFile(stream, fileName);
    }
    ...
    public List<PushbulletDevice> GetDeviceList()
    {
        ...
        return devicesResponse != null && devicesResponse.devices != null
            ? devicesResponse.devices.Where(x => !string.IsNullOrEmpty(x.nickname)).Select(x1 => new PushbulletDevice { Key = x1.iden, Name = x1.nickname }).ToList()
            : new List<PushbulletDevice>();
    }
...
public class PushbulletSettings
{
    ...
    public PushbulletDevice CurrentDevice
    {
        get
        {
            return DeviceList.IsValidIndex(SelectedDevice) ? DeviceList[SelectedDevice] : null;
        }
    }
}