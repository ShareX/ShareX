...
public partial class CustomUploaderSettingsForm : Form
{
    ...
    public CustomUploaderSettingsForm(UploadersConfig config)
    {
        ...
        */
        ...
        CodeMenuItem[] outputCodeMenuItems = new CodeMenuItem[]
        {
            ...
            new("{inputbox:title|default_value}", "Lets user to input text"),
            new("{outputbox:title|text}", "Lets user to output text"),
    ...
    private CustomUploaderItem CustomUploaderGetSelected()
    {
        ...
        return CustomUploaderCheck(index) ? Config.CustomUploadersList[index] : null;
    }
    ...
    private void CheckDataGridView(DataGridView dgv, bool checkDuplicate)
    {
        for (int i = dgv.Rows.Count - 1; i > -1; i--)
        {
            ...
            } else if (checkDuplicate)
            {
                ...
                cell.ErrorText = isDuplicate ? Resources.DuplicateNameNotAllowed : null;
            }
        }
    }
    ...
    private void CustomUploaderDestinationTypeUpdate()
    {
        ...
        if (uploader != null)
        {
            mbDestinationType.Text = uploader.DestinationType == CustomUploaderDestinationType.None
                ? CustomUploaderDestinationType.None.GetLocalizedDescription()
                : string.Join(", ", uploader.DestinationType.GetFlags().Select(x => x.GetLocalizedDescription()));
        }
    }
            ...
            try
            {
                switch (type)
                {
                    case CustomUploaderDestinationType.ImageUploader:
                        ...
                        {
                            CustomImageUploader imageUploader = new(item);
                    ...
                    case CustomUploaderDestinationType.TextUploader:
                        ...
                        using (TextUploadForm form = new("ShareX text upload test"))
                        {
                    ...
                    case CustomUploaderDestinationType.FileUploader:
                        using (Stream stream = ShareXResources.Logo.GetStream())
                        {
                            CustomFileUploader fileUploader = new(item);
                    ...
                    case CustomUploaderDestinationType.URLShortener:
                        CustomURLShortener urlShortener = new(item);
                    ...
                    case CustomUploaderDestinationType.URLSharingService:
                        CustomURLSharer urlSharer = new(item);
    ...
    private void CustomUploaderSettingsForm_DragEnter(object sender, DragEventArgs e)
    {
        e.Effect = e.Data.GetDataPresent(DataFormats.FileDrop, false)
            ? e.Data.GetData(DataFormats.FileDrop, false) is string[] files && files.Any(x => !string.IsNullOrEmpty(x) && x.EndsWith(".sxcu"))
    ...
    {
        using CustomUploaderSyntaxTestForm syntaxTestForm = new(lastResult?.ResponseInfo, rtbResultURL.Text);
