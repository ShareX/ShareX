...
public partial class UploadersConfigForm
{
    ...
    private void FlickrAuthOpen()
    {
        try
        {
            OAuthInfo oauth = new(APIKeys.APIKeys.FlickrKey, APIKeys.APIKeys.FlickrSecret);
    ...
    public void PhotobucketAuthOpen()
    {
        try
        {
            OAuthInfo oauth = new(APIKeys.APIKeys.PhotobucketConsumerKey, APIKeys.APIKeys.PhotobucketConsumerSecret);
    ...
    public void PhotobucketAuthComplete()
    {
        try
        {
            ...
            {
                Photobucket pb = new(Config.PhotobucketOAuthInfo);
    ...
    {
        Photobucket pb = new(Config.PhotobucketOAuthInfo, Config.PhotobucketAccountInfo);
    ...
    {
        AmazonS3 s3 = new(Config.AmazonS3Settings);
    ...
    {
        GoogleCloudStorage gcs = new(Config.GoogleCloudStorageOAuth2Info)
        {
    ...
    {
        AzureStorage azure = new(Config.AzureStorageAccountName, Config.AzureStorageAccountAccessKey, Config.AzureStorageContainer,
        ...
        try
        {
            ...
            if (OAuth2Info.CheckOAuth(Config.GoogleDriveOAuth2Info))
            {
                ...
                if (folders != null)
                {
                    ...
                    {
                        ListViewItem lvi = new(folder.name);
    ...
    public void BoxListFolders(BoxFileEntry fileEntry)
    {
        ...
        {
            Box box = new(Config.BoxOAuth2Info);
    ...
    public void OneDriveListFolders(OneDriveFileInfo fileEntry, TreeNode tnParent)
    {
        ...
        OneDrive oneDrive = new(Config.OneDriveV2OAuth2Info);
    ...
    private FTPAccount FTPGetSelectedAccount()
    {
        ...
        return FTPCheckAccount(index) ? Config.FTPAccountList[index] : null;
    }
        ...
        try
        {
            if (account.Protocol == FTPProtocol.FTP || account.Protocol == FTPProtocol.FTPS)
            {
                ...
                if (ftp.Connect())
                {
                    ...
                    if (ftp.IsConnected)
                    {
                        msg = directories.Count > 0
                            ? Resources.UploadersConfigForm_TestFTPAccount_Connected_Created_folders + "\r\n" + string.Join("\r\n", directories)
                            : Resources.UploadersConfigForm_TestFTPAccount_Connected_;
                    }
                }
            } else if (account.Protocol == FTPProtocol.SFTP)
            {
                using SFTP sftp = new(account);
                if (sftp.Connect())
                {
    ...
    {
        UserPassBox upb = new(Resources.UploadersConfigForm_SendSpaceRegister_SendSpace_Registration___, "John Doe", "john.doe@gmail.com", "JohnDoe", "");

        if (upb.ShowDialog() == DialogResult.OK)
        {
            SendSpace sendSpace = new(APIKeys.APIKeys.SendSpaceKey);
    ...
    public void PastebinLogin()
    {
        if (Config.PastebinSettings != null)
        {
            try
            {
                Pastebin pastebin = new(APIKeys.APIKeys.PastebinKey, Config.PastebinSettings);
    ...
    {
        lblPastebinLoginStatus.Text = Config.PastebinSettings == null || string.IsNullOrEmpty(Config.PastebinSettings.UserKey)
            ? Resources.UploadersConfigForm_UpdatePastebinStatus_NotLoggedIn
            : Resources.UploadersConfigForm_UpdatePastebinStatus_LoggedIn;
    }
    ...
    public void PushbulletGetDevices()
    {
        ...
        Pushbullet pushbullet = new(Config.PushbulletSettings);
    ...
    private bool TwitterUpdateSelected()
    {
        ...
        if (Config.TwitterSelectedAccount > -1)
        {
            ...
            if (oauth != null)
            {
                ...
                oauthTwitter.Status = OAuthInfo.CheckOAuth(oauth) ? OAuthLoginStatus.LoginSuccessful : OAuthLoginStatus.LoginRequired;
    ...
    private void TwitterAuthOpen()
    {
        if (CheckTwitterAccounts())
        {
            try
            {
                OAuthInfo oauth = new(APIKeys.APIKeys.TwitterConsumerKey, APIKeys.APIKeys.TwitterConsumerSecret);
    ...
    public void JiraAuthOpen()
    {
        try
        {
            OAuthInfo oauth = new(APIKeys.APIKeys.JiraConsumerKey);
    ...
    public void JiraAuthComplete(string code)
    {
        try
        {
            ...
            {
                Jira jira = new(Config.JiraHost, Config.JiraOAuthInfo);
