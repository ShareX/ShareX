...
public static class CaptureHelpers
{
    ...
    {
        return posCurrent.X > posOnClick.X
            ? posCurrent.Y > posOnClick.Y
                ? new PointF(posOnClick.X + size.Width - 1, posOnClick.Y + size.Height - 1)
                : new PointF(posOnClick.X + size.Width - 1, posOnClick.Y - size.Height + 1)
            : posCurrent.Y > posOnClick.Y
                ? new PointF(posOnClick.X - size.Width + 1, posOnClick.Y + size.Height - 1)
                : new PointF(posOnClick.X - size.Width + 1, posOnClick.Y - size.Height + 1);
    ...
    public static bool IsActiveWindowFullscreen()
    {
        ...
        if (handle.ToInt32() > 0)
        {
            ...
            string[] ignoreList = ["Progman", "WorkerW"];
