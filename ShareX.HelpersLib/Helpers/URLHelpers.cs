...
public static class URLHelpers
{
    ...
    public static string URLEncode(string text, bool isPath = false, bool ignoreEmoji = false)
    {
        ...
        if (!string.IsNullOrEmpty(text))
        {
            ...
            foreach (char c in Encoding.UTF8.GetBytes(text).Select(v => (char)v))
            {
    ...
    public static string HtmlEncode(string text)
    {
        ...
        StringBuilder result = new(chars.Length + (int)(chars.Length * 0.1));
    ...
    {
        return !string.IsNullOrEmpty(url) && !HasPrefix(url) ? prefix + url : url;
    }
    ...
    public static string CreateQueryString(Dictionary<string, string> args, bool customEncoding = false)
    {
        if (args != null && args.Count > 0)
        {
            ...
            foreach (KeyValuePair<string, string> arg in args)
            {
                ...
                {
                    string value = customEncoding ? URLEncode(arg.Value) : HttpUtility.UrlEncode(arg.Value);
                    pair = arg.Key + "=" + value;
                }
    ...
    public static string CreateQueryString(string url, Dictionary<string, string> args, bool customEncoding = false)
    {
        ...
        return !string.IsNullOrEmpty(query) ? url.Contains("?") ? url + "&" + query : url + "?" + query : url;
    }
