...
public static class Helpers
{
    public const string
        ...
        AlphabetCapital = "ABCDEFGHIJKLMNOPQRSTUVWXYZ", // 65 ... 90
        Alphabet = "abcdefghijklmnopqrstuvwxyz", // 97 ... 122
        Alphanumeric = Numbers + AlphabetCapital + Alphabet,
        AlphanumericInverse = Numbers + Alphabet + AlphabetCapital,
        Hexadecimal = Numbers + "ABCDEF",
        Base58 = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz", // https://en.wikipedia.org/wiki/Base58
        Base56 = "23456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnpqrstuvwxyz"; // A variant, Base56, excludes 1 (one) and o (lowercase o) compared to Base 58.
    ...
    {
        return hour == 0 ? 12.ToString() : hour > 12 ? AddZeroes(hour - 12) : AddZeroes(hour);
    }
    ...
    {
        return Encoding.UTF8.GetString([.. Enumerable.Range(1, 255).Select(i => (byte)i)]);
    }
    ...
    public static string GetRandomLine(string text)
    {
        ...
        return lines != null && lines.Length > 0 ? RandomCrypto.Pick(lines) : null;
    }
    ...
    {
        return [.. Enum.GetValues(typeof(T)).OfType<Enum>().Skip(skip).Select(x => x.GetDescription())];
    }
    ...
    {
        return [.. Enum.GetValues(typeof(T)).OfType<Enum>().Select(x => x.GetLocalizedDescription(resourceManager))];
    }
    ...
    public static void PlaySoundAsync(Stream stream)
    {
        if (stream != null)
        {
            ...
            {
                using (stream)
                using (SoundPlayer soundPlayer = new(stream))
                {
    ...
    public static Point GetPosition(ContentAlignment placement, Point offset, Size backgroundSize, Size objectSize)
    {
        ...
        {
            ContentAlignment.TopCenter => new Point(midX, offset.Y),
            ContentAlignment.TopRight => new Point(right - offset.X, offset.Y),
            ContentAlignment.MiddleLeft => new Point(offset.X, midY),
            ContentAlignment.MiddleCenter => new Point(midX, midY),
            ContentAlignment.MiddleRight => new Point(right - offset.X, midY),
            ContentAlignment.BottomLeft => new Point(offset.X, bottom - offset.Y),
            ContentAlignment.BottomCenter => new Point(midX, bottom - offset.Y),
            ContentAlignment.BottomRight => new Point(right - offset.X, bottom - offset.Y),
    ...
    public static string SendPing(string host, int count)
    {
        ...
        using (Ping ping = new())
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                status[i] = reply.Status == IPStatus.Success ? reply.RoundtripTime.ToString() + " ms" : "Timeout";
                Thread.Sleep(100);
            }
        }
    ...
    public static bool IsMemberOfAdministratorsGroup()
    {
        try
        {
            using WindowsIdentity identity = WindowsIdentity.GetCurrent();
            WindowsPrincipal principal = new(identity);
            SecurityIdentifier sid = new(WellKnownSidType.BuiltinAdministratorsSid, null);
    ...
    {
        BufferedStream bufferedStream = new(stream, bufferSize);
    ...
    {
        using HMACSHA256 hashAlgorithm = new(key);
        return hashAlgorithm.ComputeHash(data);
    }
    ...
    public static string SafeStringFormat(IFormatProvider provider, string format, params object[] args)
    {
        try
        {
            return provider != null ? string.Format(provider, format, args) : string.Format(format, args);
        } catch (Exception e)
        {
    ...
    public static bool TryFixHandCursor()
    {
        try
        {
            FieldInfo handField = typeof(Cursors).GetField("hand", BindingFlags.NonPublic | BindingFlags.Static);
            if (handField != null)
            {
                handField.SetValue(null, new Cursor(NativeMethods.LoadCursor(IntPtr.Zero, NativeConstants.IDC_HAND)));
                return true;
            }
            return false;
        } catch
    ...
    public static string XMLFormat(string xml)
    {
        ...
        using XmlTextWriter writer = new(ms, Encoding.Unicode);
    ...
    public static Icon GetProgressIcon(int percentage, Color color)
    {
        ...
        if (y > 0)
        {
            ...
            if (y < size.Height)
            {
                using Pen pen = new(ColorHelpers.LighterColor(color, 0.3f));
    ...
    {
        SemaphoreSlim throttler = new(maxDegreeOfParallelism, maxDegreeOfParallelism);
    ...
    {
        return current == null || !current.Any() || current.All(x => source.Any(y => predicate(x, y)));
    }
