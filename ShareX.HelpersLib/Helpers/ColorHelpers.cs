...
public static class ColorHelpers
{
    ...
    public static string ColorToHex(Color color, ColorFormat format = ColorFormat.RGB)
    {
        ...
        {
            ColorFormat.RGBA => string.Format("{0:X2}{1:X2}{2:X2}{3:X2}", color.R, color.G, color.B, color.A),
            ColorFormat.ARGB => string.Format("{0:X2}{1:X2}{2:X2}{3:X2}", color.A, color.R, color.G, color.B),
            _ => string.Format("{0:X2}{1:X2}{2:X2}", color.R, color.G, color.B),
        };
    }
    ...
    public static int ColorToDecimal(Color color, ColorFormat format = ColorFormat.RGB)
    {
        return format switch
        {
            ColorFormat.RGBA => color.R << 24 | color.G << 16 | color.B << 8 | color.A,
            ColorFormat.ARGB => color.A << 24 | color.R << 16 | color.G << 8 | color.B,
            _ => color.R << 16 | color.G << 8 | color.B,
        };
    }
    ...
    public static HSB ColorToHSB(Color color)
    {
        ...
        hsb.Saturation = Max == 0 ? 0 : (double)Diff / Max;

        double q = Diff == 0 ? 0 : (double)60 / Diff;
        hsb.Hue = Max == color.R
            ? color.G < color.B ? (360 + q * (color.G - color.B)) / 360 : q * (color.G - color.B) / 360
            : Max == color.G ? (120 + q * (color.B - color.R)) / 360 : Max == color.B ? (240 + q * (color.R - color.G)) / 360 : 0.0;
    ...
    public static Color DecimalToColor(int dec, ColorFormat format = ColorFormat.RGB)
    {
        return format switch
        {
            ColorFormat.RGBA => Color.FromArgb(dec & 0xFF, dec >> 24 & 0xFF, dec >> 16 & 0xFF, dec >> 8 & 0xFF),
            ColorFormat.ARGB => Color.FromArgb(dec >> 24 & 0xFF, dec >> 16 & 0xFF, dec >> 8 & 0xFF, dec & 0xFF),
            _ => Color.FromArgb(dec >> 16 & 0xFF, dec >> 8 & 0xFF, dec & 0xFF),
        };
    }
