...
public static class ImageHelpers
{
    ...
    public static Bitmap ResizeImage(Bitmap bmp, int width, int height, InterpolationMode interpolationMode = DefaultInterpolationMode)
    {
        ...
        Bitmap bmpResult = new(width, height, PixelFormat.Format32bppArgb);
    ...
    {
        Bitmap bmp = new(bitmapSource.PixelWidth, bitmapSource.PixelHeight, pixelFormat);
    ...
    public static Bitmap CreateThumbnail(Bitmap bmp, int width, int height)
    {
        ...
        {
            w = srcRatio >= 1 ? (int)(bmp.Height * dstRatio) : (int)(bmp.Width / srcRatio * dstRatio);
        ...
        } else
        {
            ...
            h = srcRatio >= 1 ? (int)(bmp.Height / dstRatio * srcRatio) : (int)(bmp.Height * srcRatio / dstRatio);
        }
    ...
    {
        return bmp != null && rect.X >= 0 && rect.Y >= 0 && rect.Width > 0 && rect.Height > 0 && new Rectangle(0, 0, bmp.Width, bmp.Height).Contains(rect)
    ...
    private static Bitmap ApplyCutOutEffect(Bitmap bmp, AnchorStyles effectEdge, CutOutEffectType effectType, int effectSize, Color backgroundColor)
    {
        ...
        {
            CutOutEffectType.None => bmp,
            CutOutEffectType.ZigZag => TornEdges(bmp, effectSize, effectSize, effectEdge, false, false, backgroundColor),
            CutOutEffectType.TornEdge => TornEdges(bmp, effectSize, effectSize * 2, effectEdge, false, true, backgroundColor),
            CutOutEffectType.Wave => WavyEdges(bmp, effectSize, effectSize * 5, effectEdge, backgroundColor),
    ...
    public static Bitmap CutOutBitmapMiddle(Bitmap bmp, Orientation orientation, int start, int size, CutOutEffectType effectType, int effectSize, Color backgroundColor)
    {
        if (bmp != null && size > 0)
        {
            ...
            {
                return CombineImages([firstPart, secondPart], orientation);
    ...
    {
        Rectangle source = new(0, 0, bmp.Width, bmp.Height);
        Rectangle rect = source;

        using (UnsafeBitmap unsafeBitmap = new(bmp, true, ImageLockMode.ReadOnly))
        {
            bool leave = false;
    ...
    public static Bitmap RoundedCorners(Bitmap bmp, int cornerRadius)
    {
        ...
        using (Graphics g = Graphics.FromImage(bmpResult))
        {
            ...
            using TextureBrush brush = new(bmp);
            g.FillPath(brush, gp);
        }
            ...
            for (int x = 0; x < source.Width; x++)
            {
                for (int y = 0; y < source.Height; y++)
                {
                    ...
                    if (dist > minRadius && dist < maxRadius)
                    {
                        ...
                        ColorBgra bgra = new(color.B, color.G, color.R, alpha);
    ...
    public static Bitmap DrawReflection(Bitmap bmp, int percentage, int maxAlpha, int minAlpha, int offset, bool skew, int skewSize)
    {
        ...
        Bitmap bmpResult = new(reflection.Width, bmp.Height + reflection.Height + offset);
    ...
    public static Bitmap AddSkew(Image img, int x, int y)
    {
        ...
        using (Graphics g = Graphics.FromImage(result))
        {
            ...
            Point[] destinationPoints = [new(startX, startY), new(startX + img.Width - 1, endY), new(endX, startY + img.Height - 1)];
    ...
    public static Bitmap AddReflection(Image img, int percentage, int maxAlpha, int minAlpha)
    {
        ...
        using (UnsafeBitmap unsafeBitmap = new(reflection, true))
        {
    ...
    {
        using Pen borderPen = new(borderColor, borderSize) { Alignment = PenAlignment.Inset, DashStyle = dashStyle };
        ...
        DashStyle dashStyle = DashStyle.Solid)
    {
        ...
        using LinearGradientBrush brush = new(new Rectangle(0, 0, width, height), fromBorderColor, toBorderColor, gradientType);
        using Pen borderPen = new(brush, borderSize) { Alignment = PenAlignment.Inset, DashStyle = dashStyle };
        return DrawBorder(bmp, borderPen, borderType);
    }
    ...
    {
        using LinearGradientBrush brush = new(new Rectangle(0, 0, img.Width, img.Height), fromColor, toColor, gradientType);
        return FillBackground(img, brush);
    }
    ...
    {
        Bitmap bmp = new(width * 2, height * 2);
    ...
    public static bool AddMetadata(Image img, int id, string text)
    {
        try
        {
            PropertyItem pi = (PropertyItem)typeof(PropertyItem).GetConstructor(BindingFlags.Instance | BindingFlags.NonPublic, null, [], null).Invoke(null);
            pi.Id = id;
    ...
    public static Bitmap AddShadow(Bitmap bmp, float opacity, int size, float darkness, Color color, Point offset, bool autoResize = true)
    {
        ...
        try
        {
            ...
            Rectangle shadowRectangle = new(size, size, bmp.Width, bmp.Height);
    ...
    public static Bitmap AddGlow(Bitmap bmp, int size, float strength, Color color, Point offset, GradientInfo gradient = null)
    {
        ...
        try
        {
            ...
            bmpMask = gradient != null && gradient.IsValid
                ? CreateGradientMask(bmpBlur, gradient, strength)
                : ColorMatrixManager.Mask(strength, color).Apply(bmpBlur);
    ...
    public static Bitmap CreateGradientMask(Bitmap bmp, GradientInfo gradient, float opacity = 1f)
    {
        ...
        using (UnsafeBitmap bmpSource = new(bmp, true, ImageLockMode.ReadOnly))
        using (UnsafeBitmap bmpMask = new(mask, true, ImageLockMode.ReadWrite))
        {
    ...
    {
        using UnsafeBitmap unsafeBitmap = new(bmp, true);
        for (int y = rect.Y; y < rect.Height; y++)
        {
            for (int x = rect.X; x < rect.Width; x++)
            {
        ...
        if (pixelSize > 1)
        {
            ...
            for (int y = 0; y < unsafeBitmap.Height; y += pixelSize)
            {
                for (int x = 0; x < unsafeBitmap.Width; x += pixelSize)
                {
                    ...
                    ColorBgra averageColor = new((byte)(b / weightedCount), (byte)(g / weightedCount), (byte)(r / weightedCount), (byte)(a / pixelCount));
    ...
    public static void Pixelate(Bitmap bmp, int pixelSize, int borderSize, Color borderColor)
    {
        ...
        {
            using Bitmap bmpTexture = new(pixelSize, pixelSize);
            using (Graphics g = Graphics.FromImage(bmpTexture))
            using (Pen pen = new(borderColor, borderSize) { Alignment = PenAlignment.Inset })
            {
                g.DrawRectangleProper(pen, new Rectangle(0, 0, bmpTexture.Width, bmpTexture.Height));
            }

            using (Graphics g = Graphics.FromImage(bmp))
            using (TextureBrush brush = new(bmpTexture))
            {
    ...
    public static Bitmap GaussianBlur(Bitmap bmp, int radius)
    {
        ...
        ConvolutionMatrix kernelVertical = new(size, 1)
        {
    ...
    public static void ColorDepth(Bitmap bmp, int bitsPerChannel = 4)
    {
        ...
        static byte Remap(byte color, double interval)
        {
    ...
    public static void FastBoxBlur(Bitmap bmp, int radius)
    {
        ...
        for (x = 0; x < w; x++)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ColorBgra color = new(dv[bsum], dv[gsum], dv[rsum], dv[asum]);
    ...
    public static Bitmap WavyEdges(Bitmap bmp, int waveDepth, int waveRange, AnchorStyles sides, Color backgroundColor)
    {
        ...
        static int waveFunction(int t, int max, int depth) => (int)((1 - Math.Cos(t * Math.PI / max)) * depth / 2);
        ...
        {
            points.Add(new Point(bmp.Width, points[^1].Y));
        }
        ...
        {
            points.Add(new Point(points[^1].X, bmp.Height));
        }
        ...
        {
            points.Add(new Point(0, points[^1].Y));
        }

        if (!sides.HasFlag(AnchorStyles.Top))
        {
            points[0] = new Point(points[^1].X, 0);
        }
    ...
    public static Bitmap TornEdges(Bitmap bmp, int tornDepth, int tornRange, AnchorStyles sides, bool curvedEdges, bool random, Color backgroundColor)
    {
        ...
        using (TextureBrush brush = new(bmp))
        {
            ...
            Point[] fillPoints = [.. points.Distinct()];
    ...
    {
        ArgumentOutOfRangeException.ThrowIfLessThan(minSliceHeight, 1);

        ArgumentOutOfRangeException.ThrowIfLessThan(maxSliceHeight, 1);
        ...
        using (Graphics g = Graphics.FromImage(bmpResult))
        {
            ...
            while (y < bmp.Height)
            {
                Rectangle sourceRect = new(0, y, bmp.Width, RandomFast.Next(minSliceHeight, maxSliceHeight));
                Rectangle destRect = sourceRect;

                destRect.X = RandomFast.Next(1) == 0 ? RandomFast.Next(-maxSliceShift, -minSliceShift) : RandomFast.Next(minSliceShift, maxSliceShift);
    ...
    public static string[] OpenImageFileDialog(bool multiselect, Form form = null, string initialDirectory = null)
    {
        ...
        return ofd.ShowDialog(form) == DialogResult.OK ? ofd.FileNames : null;
    }
    ...
    public static Bitmap LoadImageWithFileDialog(Form form = null)
    {
        ...
        return !string.IsNullOrEmpty(filePath) ? LoadImage(filePath) : null;
    }
        ...
        int space = 0, int wrapAfter = 0, bool autoFillBackground = false)
    {
        ...
        Bitmap bmp = new(totalImageRect.Width, totalImageRect.Height);
        ...
        for (int y = 0; y < rowCount; y++)
        {
            for (int x = 0; x < columnCount; x++)
            {
                ...
                using (Graphics g = Graphics.FromImage(bmp))
                {
                    ...
                    Rectangle srcRect = new(x * width, y * height, width, height);
    ...
    public static void DrawColorPickerIcon(Graphics g, Color color, Rectangle rect, int holeSize = 0)
    {
        ...
        if (holeSize > 0)
        {
            ...
            Rectangle holeRect = new(rect.Width / 2 - holeSize / 2, rect.Height / 2 - holeSize / 2, holeSize, holeSize);
    ...
    {
        ColorBgra sourceBgra = new(sourceColor);
        ColorBgra targetBgra = new(targetColor);
    ...
    public static InterpolationMode GetInterpolationMode(ImageInterpolationMode interpolationMode)
    {
        ...
        {
            ImageInterpolationMode.Bicubic => InterpolationMode.Bicubic,
            ImageInterpolationMode.HighQualityBilinear => InterpolationMode.HighQualityBilinear,
            ImageInterpolationMode.Bilinear => InterpolationMode.Bilinear,
            ImageInterpolationMode.NearestNeighbor => InterpolationMode.NearestNeighbor,
    ...
    public static void SavePNG(Image img, Stream stream, PNGBitDepth bitDepth)
    {
        ...
        {
            bitDepth = IsImageTransparent((Bitmap)img) ? PNGBitDepth.Bit32 : PNGBitDepth.Bit24;
        }
