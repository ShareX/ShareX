...
All 129 keys are translated in all 27 supported cultures. The existing `Properties/Resources.resx` catalog remains responsible for its 390 already-translated legacy strings and 11 bitmap/icon resources. The `Controls` and `Colors` directories are intentionally excluded from this localization catalog.
