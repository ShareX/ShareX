...
public static class ZipManager
{
        ...
        long maxUncompressedSize = 0)
    {
        ...
        foreach (ZipArchiveEntry entry in archive.Entries)
        {
            ...
            string entryName = retainDirectoryStructure ? entry.FullName : entry.Name;
    ...
    private static void ExtractToFile(ZipArchiveEntry source, string destinationFileName, bool overwrite)
    {
        ...
        using (FileStream fs = new(destinationFileName, fMode, FileAccess.Write, FileShare.None, bufferSize: 0x1000, useAsync: false))
        using (Stream es = source.Open())
        using (MaxLengthStream maxLengthStream = new(es, source.Length))
        {
