...
public struct MyColor(Color color)
{
    public RGBA RGBA = color;
    public HSB HSB = color;
    public CMYK CMYK = color;

    public bool IsTransparent => RGBA.Alpha < 255;

    public static implicit operator MyColor(Color color) => new(color);

    public static implicit operator Color(MyColor color) => color.RGBA;

    public static bool operator ==(MyColor left, MyColor right) => left.RGBA == right.RGBA && left.HSB == right.HSB && left.CMYK == right.CMYK;
