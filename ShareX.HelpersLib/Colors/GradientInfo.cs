...
public class GradientInfo(LinearGradientMode type)
{
    ...
    {
        List<GradientStop> colors = [.. Colors.OrderBy(x => x.Location)];
        ...
        {
            colors.Add(new GradientStop(colors[^1].Color, 100f));
        }

        ColorBlend colorBlend = new();
        colorBlend.Colors = [.. colors.Select(x => x.Color)];
        colorBlend.Positions = [.. colors.Select(x => x.Location / 100)];
        return colorBlend;
    }

    public LinearGradientBrush GetGradientBrush(Rectangle rect)
    {
        LinearGradientBrush brush = new(rect, Color.Transparent, Color.Transparent, Type);
