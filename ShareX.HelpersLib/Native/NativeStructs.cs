...
public struct RECT(int left, int top, int right, int bottom)
{
    ...
    {
        return obj is RECT rect ? Equals(rect) : obj is Rectangle rectangle && Equals(new RECT(rectangle));
    }

    public override readonly int GetHashCode() => ((Rectangle)this).GetHashCode();

    public override readonly string ToString() => string.Format(CultureInfo.CurrentCulture, "{{Left={0},Top={1},Right={2},Bottom={3}}}", Left, Top, Right, Bottom);
}
...
public struct SIZE(int width, int height)
{
    ...
    public override readonly string ToString() => string.Format("{0}x{1}", Width, Height);
}
...
public struct BITMAPINFOHEADER
{
    ...
    public readonly uint OffsetToPixels => biCompression == BI_COMPRESSION.BI_BITFIELDS ? biSize + 3 * 4 : biSize;
}

[StructLayout(LayoutKind.Sequential)]
public struct CIEXYZ(uint FXPT2DOT30)
{
