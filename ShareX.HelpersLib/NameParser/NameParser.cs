...
public class NameParser
{
    ...
    public string Parse(string pattern)
    {
        ...
        string hour = sb.ToString().Contains(CodeMenuEntryFilename.pm.ToPrefixString())
            ? Helpers.Helpers.HourTo12(dt.Hour)
            : Helpers.Helpers.AddZeroes(dt.Hour);
        ...
        foreach (Tuple<string, string> entry in ListEntryWithArgument(result, CodeMenuEntryFilename.rf.ToPrefixString()))
        {
            result = result.ReplaceAll(entry.Item1, () =>
            {
                try
                {
                    ...
                    return FileHelpers.IsTextFile(path)
                        ? Helpers.Helpers.GetRandomLineFromFile(path)
    ...
    private static IEnumerable<Tuple<string, string>> ListEntryWithArgument(string text, string entry)
    {
