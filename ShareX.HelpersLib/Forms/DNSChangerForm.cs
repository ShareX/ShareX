    ...
    private void cbAdapters_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (cbAdapters.SelectedItem is AdapterInfo adapter)
        {
            ...
            if (dns != null && dns.Length > 0)
            {
                ...
                txtAlternateDNS.Text = dns.Length > 1 ? dns[1] : "";
            } else
            {
        ...
        if (cbAdapters.SelectedItem is AdapterInfo adapter)
        {
            ...
            try
            {
                ...
                } else
                {
                    ...
                    result = Helpers.Helpers.IsValidIPAddress(primaryDNS) && (string.IsNullOrEmpty(secondaryDNS) || Helpers.Helpers.IsValidIPAddress(secondaryDNS))
                        ? adapter.SetDNS(primaryDNS, secondaryDNS)
