...
public partial class ColorPickerForm : Form
{
    ...
    {
        using (ColorPickerForm dialog = new(currentColor, options: options))
        {
    ...
    private void PrepareColorPalette()
    {
        ...
        Color[] colors = Options.RecentColorsSelected ? HelpersOptions.RecentColors.ToArray() : ColorHelpers.StandardColors;
    ...
    {
        Bitmap bmp = new(pbColorPreview.ClientSize.Width, pbColorPreview.ClientSize.Height);
        ...
        using (Graphics g = Graphics.FromImage(bmp))
        {
            ...
            if (oldColorExist)
            {
                ...
                using SolidBrush oldColorBrush = new(OldColor);
                g.FillRectangle(oldColorBrush, new Rectangle(0, bmpHeight, bmp.Width, bmpHeight));
            }

            using SolidBrush newColorBrush = new(NewColor);
