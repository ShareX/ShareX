...
public static class GraphicsExtensions
{
    ...
    public static void DrawRectangleShadow(this Graphics g, Rectangle rect, Color shadowColor, int shadowDepth, int shadowMaxAlpha, int shadowMinAlpha, Padding shadowDirection)
    {
        for (int i = 0; i < shadowDepth; i++)
        {
            ...
            if (currentAlpha > 0)
            {
                using Pen pen = new(Color.FromArgb(currentAlpha, shadowColor));
                Rectangle shadowRect = new(rect.X + -shadowDirection.Left * i, rect.Y + -shadowDirection.Top * i,
    ...
    {
        g.DrawRoundedRectangle(brush, pen, new Rectangle(x, y, width, height), radius);
    }
    ...
    {
        g.FillRoundedRectangle(brush, new Rectangle(x, y, width, height), radius);
    }
    ...
    public static void DrawTextWithOutline(this Graphics g, string text, PointF position, Font font, Color textColor, Color borderColor, int borderSize = 2)
    {
        ...
        using (GraphicsPath gp = new())
        {
            ...
            if (borderSize > 0)
            {
                using Pen borderPen = new(borderColor, borderSize) { LineJoin = LineJoin.Round };
