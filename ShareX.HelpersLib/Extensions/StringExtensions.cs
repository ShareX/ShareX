...
public static class StringExtensions
{
    ...
    {
        return length < 1 ? "" : length < str.Length ? str.Substring(0, length) : str;
    }

    public static string Right(this string str, int length)
    {
        return length < 1 ? "" : length < str.Length ? str.Substring(str.Length - length) : str;
    }

    public static string RemoveLeft(this string str, int length)
    {
        return length < 1 ? "" : length < str.Length ? str.Remove(0, length) : str;
    }

    public static string RemoveRight(this string str, int length)
    {
        return length < 1 ? "" : length < str.Length ? str.Remove(str.Length - length) : str;
    }
    ...
    {
        return !string.IsNullOrEmpty(str) && str.Length > maxLength ? str.Substring(0, maxLength) : str;
    }
    ...
    public static string Truncate(this string str, int maxLength, string endings, bool truncateFromRight = true)
    {
        if (!string.IsNullOrEmpty(str) && str.Length > maxLength)
        {
            ...
            if (length > 0)
            {
                str = truncateFromRight ? str.Left(length) + endings : endings + str.Right(length);
            }
        }
