...
{
    public const uint DefaultPolynomial = 0xedb88320, DefaultSeed = 0xffffffff;
    private uint hash,seed;
    private uint[] table;
    ...
    protected override void HashCore(byte[] buffer, int start, int length) => hash = CalculateHash(table, hash, buffer, start, length);
