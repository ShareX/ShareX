...
namespace ShareX.HelpersLib.Controls.BlackStyle;
...
public class BlackStyleProgressBar : Control
{
    ...
    {
        using (LinearGradientBrush backgroundBrush = new(new Rectangle(1, 1, ClientSize.Width - 2, ClientSize.Height - 2),
        ...
        using (Pen borderShadowPen = new(Color.FromArgb(45, 45, 45)))
        {
        ...
        using Pen borderPen = new(Color.FromArgb(30, 30, 30));
    ...
    private void DrawProgressBar(Graphics g)
    {
        ...
        Rectangle progressBarRect = new(1, 1, (int)progressBarSize, ClientSize.Height - 2);

        using (LinearGradientBrush progressBarBrush = new(progressBarRect, Color.Black, Color.Black, LinearGradientMode.Vertical))
        {
            ColorBlend cb = new();
            cb.Positions = [0, 0.49f, 0.50f, 1];
            cb.Colors = [Color.FromArgb(102, 163, 226), Color.FromArgb(83, 135, 186), Color.FromArgb(75, 121, 175), Color.FromArgb(56, 93, 135)];
            progressBarBrush.InterpolationColors = cb;

            g.FillRectangle(progressBarBrush, progressBarRect);
        }

        using LinearGradientBrush innerBorderBrush = new(progressBarRect, Color.FromArgb(133, 192, 241), Color.FromArgb(76, 119, 163), LinearGradientMode.Vertical);
        using Pen innerBorderPen = new(innerBorderBrush);
