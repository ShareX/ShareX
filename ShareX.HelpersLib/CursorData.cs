...
public class CursorData
{
    ...
    public Point DrawPosition => new(Position.X - Hotspot.X, Position.Y - Hotspot.Y);
        ...
        if (NativeMethods.GetCursorInfo(out cursorInfo))
        {
            ...
            if (IsVisible)
            {
                ...
                if (iconHandle != IntPtr.Zero)
                {
                    ...
                    {
                        Hotspot = IsDefaultSize
                            ? new Point(iconInfo.xHotspot, iconInfo.yHotspot)
                            : new Point((int)Math.Round(iconInfo.xHotspot * SizeMultiplier), (int)Math.Round(iconInfo.yHotspot * SizeMultiplier));
    ...
    public void DrawCursor(IntPtr hdcDest, Point offset)
    {
        if (IsVisible)
        {
            Point drawPosition = new(DrawPosition.X - offset.X, DrawPosition.Y - offset.Y);
